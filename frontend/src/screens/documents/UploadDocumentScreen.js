...
const UploadDocumentScreen = ({ route, navigation }) => {
  const { orderId: prefilledOrderId } = route.params || {};
  ...
  const validateForm = () => {
    setError('');
    if (!title.trim()) { setError('Document title is required'); return false; }
    if (!selectedFile) { setError('Please select a file to upload'); return false; }
    return true;
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerTitle}>UPLOAD NEW</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Link to Order</Text>
            <View style={[styles.pickerWrapper, prefilledOrderId && { opacity: 0.7 }]}>
              <MaterialIcons name="receipt" size={18} color="#012d1d" style={styles.inputIcon} />
              <Picker
                selectedValue={orderId}
                onValueChange={setOrderId}
                style={styles.picker}
                enabled={!uploading && !prefilledOrderId}
              >
                <Picker.Item label="Standalone (No Order)" value="" color="#b0b0b0" />
                {orders.map((order) => (
                  <Picker.Item
                    key={order._id}
                ...
                {prefilledOrderId && (
                  <Picker.Item 
                    label={`Order #${prefilledOrderId.slice(-6).toUpperCase()}`} 
                    value={prefilledOrderId} 
                  />
                )}
              </Picker>
            </View>
        ...
        <View style={styles.section}>
          ...
          >
            ...
            <Text style={[styles.filePickerText, selectedFile && { color: '#012d1d' }]}>
              {selectedFile ? selectedFile.name : "Tap to select PDF or Image"}
            </Text>
            {selectedFile && (
        ...
        {uploading && (
          <View style={styles.progressSection}>
            <View style={styles.progressLabelRow}>
              <Text style={styles.progressLabel}>Uploading Document</Text>
        ...
        {/* Action Button */}
        <TouchableOpacity
          style={[styles.submitButton, (uploading || !selectedFile) && { opacity: 0.7 }]}
        ...
        >
          {uploading ? (
            ...
            <>
              ...
              <Text style={styles.submitButtonText}>Finish & Upload</Text>
            </>
          )}
        </TouchableOpacity>
